...
const setupProfile = async (req, res, next) => {
  try {
    ...
    if (Array.isArray(categories) && categories.length > 0) {
      budgetCategories = categories.map((category) => {
        const name = String(category.name || '').trim().toLowerCase();
        const amount = Number(category.allocatedAmount);
        if (!name || !amount || amount <= 0) {
          throw createError(400, 'Each budget category must have a valid name and amount');
        }
        return {
          name,
      ...
      budgetCategories = Budget.buildSuggestedCategories(monthlyIncome);
    }
