...
const goToStep2 = async () => {
  ...
  // Fetch suggested categories from backend for the entered income
  try {
    const data = await api.get(`/budget/suggestions?income=${encodeURIComponent(income)}`);
...
const setStep = (step) => {
  [1, 2, 3].forEach(n => {
    ...
    if (stepEl) stepEl.classList.toggle('hidden', n !== step);
...
const saveSetup = async () => {
  ...
  if (categories.length === 0) {
    showAlert('setup-alert', 'Please add at least one budget category', 'error');
    return;
  }

  const totalAllocated = categories.reduce((sum, c) => sum + c.allocatedAmount, 0);
  if (totalAllocated > income) {
    showAlert('setup-alert', 'Total allocated amount exceeds your income. Please adjust.', 'error');
    return;
  }

  setLoading('save-setup-btn', true);
  ...
  try {
    ...
    // Show done screen then redirect
    setStep(3);
    setTimeout(animateRedirect, 600);

  } catch (err) {
    showAlert('setup-alert', err.message || 'Setup failed. Please try again.', 'error');
  } finally {
    setLoading('save-setup-btn', false);
  }
};
